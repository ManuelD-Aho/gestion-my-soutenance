...
use App\Models\Pv;
use App\Models\User;
use App\Services\PenaltyService; // <-- AJOUTER CETTE LIGNE
                ...
                ->chunkById(200, function ($users) use (&$usersArchived) {
                    foreach ($users as $user) {
                        try {
                            ...
                            $this->userManagementService->updateUserStatus($user, UserAccountStatusEnum::ARCHIVED, "Archivage automatique car inactif depuis " . $user->last_activity_at->format('Y-m-d'));$usersArchived++;
                        } catch (Throwable $e) {
